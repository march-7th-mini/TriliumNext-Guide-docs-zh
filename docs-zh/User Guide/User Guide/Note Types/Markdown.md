...
Trilium 一直通过其[导入功能](../Basic%20Concepts%20and%20Features/Import%20%26%20Export/Markdown.md)支持 Markdown，但该文件要么被转换为<a class="reference-link" href="Text.md">文本</a>笔记（转换为 Trilium 的内部 HTML 格式），要么保存为仅具有语法高亮的<a class="reference-link" href="Code.md">代码</a>笔记。

此笔记类型为分屏视图，即源代码和文档预览并排显示。更多信息请参阅<a class="reference-link" href="../Basic%20Concepts%20and%20Features/UI%20Elements/Note%20types%20with%20split%20view.md">分屏视图的笔记类型</a>。

## 设计原理

此笔记类型的目标是填补一个空白：渲染 Markdown 但不改变其结构或空白，否则这些内容在导入/导出时不可避免地会被更改。

即使 Markdown 现在通过预览机制得到了特殊处理，Trilium 本质上仍然是一个所见即所得编辑器，因此 Markdown 不会取代文本笔记。

> [!NOTE]
> 关于 Markdown 实现的功能请求将被考虑，但如果超出 Trilium 的范围，则不会实现。Markdown 集成的核心方面之一是它复用了应用程序其他功能中已有的组件。
...
Trilium 的 Markdown 格式支持以下功能，并将显示在预览窗格中：
...
*   <a class="reference-link" href="Text/Math%20Equations.md">数学公式</a>（包括行内和块级）
*   使用 ` ```mermaid ` 的<a class="reference-link" href="Mermaid%20Diagrams.md">Mermaid 图表</a>
*   <a class="reference-link" href="Text/Include%20Note.md">包含笔记</a>（没有内置的 Markdown 语法，但 HTML 语法同样适用）：
...
*   具有扩展任务状态的待办事项列表：
    ...
    任务状态是可自定义的：您可以重新排序、创建具有不同颜色和符号的新状态，或删除不需要的状态。更多详情请参阅<a class="reference-link" href="../Advanced%20Usage/Customizing%20to-do%20task%20states.md">自定义待办任务状态</a>。
    
    请注意，具有除“None”和“Done”之外的任务状态的待办事项是 Trilium 特有的，可能无法与其他 Markdown 软件很好地兼容。
...
*   用于<a class="reference-link" href="../Basic%20Concepts%20and%20Features/Notes/Printing%20%26%20Exporting%20as%20PDF.md">打印和导出为 PDF</a> 的分页符：
    
    ```
    <div class="page-break"></div>
    ```
*   高亮（背景颜色）既支持 `==` Markdown 语法，也支持 Trilium 中的标准 HTML 表示形式：
...
*   可以使用标准 Markdown 语法编写 Web URL：
    
    ```
    [Wikipedia](https://www.wikipedia.org)
    ```
*   通过手动输入笔记 ID 或使用 _添加链接_ 对话框，创建指向其他笔记的动态标题[引用链接](Text/Links/Internal%20\(reference\)%20links.md)：
    
    ```
    [[B9oMG6rFvvfq]]
    ```
*   使用自定义文本创建指向其他笔记的[引用链接](Text/Links/Internal%20\(reference\)%20links.md)：
...
*   _剪切到笔记_ (<kbd>Ctrl</kbd>+<kbd>X</kbd>)：将选中内容剪切到新的子笔记中。
...
*   插入当前日期和时间（`/date`）。
*   [包含](Text/Include%20Note.md)另一个笔记（`/include`）。
*   上传并插入图片（`/image`）。
*   插入笔记链接（`/link`）。
*   插入数学公式块（`/math`）。
...
从 v0.104.0 开始，输入 \`\`\` 插入代码块将自动打开一个具有语法高亮的建议语言类型列表。

语言列表与<a class="reference-link" href="../Basic%20Concepts%20and%20Features/UI%20Elements/Options.md">选项</a>中设置的<a class="reference-link" href="Code.md">代码</a>笔记的语言列表一致，并非 Trilium 支持的全部语言列表。
...
前面描述的大多数功能都应受支持。如果您遇到任何问题，请随时[报告问题](../Troubleshooting/Reporting%20issues.md)，并附上示例 Markdown 文件。
...
2.  创建类型为<a class="reference-link" href="Code.md">代码</a>的笔记，并选择语言为 _Markdown_ 或 _GitHub-Flavored Markdown_。这保持了与引入此功能之前现有笔记的兼容性。
...
导入 Trilium ZIP 时，由于其中的元信息，它将保留 Markdown 类型而不会转换为文本笔记。

### 导出

导出 Markdown 文件时，扩展名会被保留，内容与源代码视图中的内容保持一致（附件处理等少数例外情况除外）。

将 Markdown 文件导出为 ZIP 时，选择 HTML 或 Markdown 作为导出格式没有区别，因为这仅影响<a class="reference-link" href="Text.md">文本</a>笔记。

如果 Markdown 笔记包含附件，ZIP 导出将重写指向附件的链接，将其替换为附件的相对路径。导入时，链接会被重新写回。
...
此外，预览中与源代码视图中光标位置匹配的块将略微高亮显示。

目前同步仅是单向的，滚动预览不会同步编辑器的位置。

此功能目前无法禁用；如果滚动感觉分散注意力，请考虑暂时切换到编辑器模式，然后在准备好时切换到预览模式。

> [!NOTE]
> 此同步滚动的功能基于块，但它是尽力提供的，因为我们底层的 Markdown 库本身不支持此功能，因此我们必须实现自己的算法。请随时[报告问题](../Troubleshooting/Reporting%20issues.md)，但请始终提供示例 Markdown 文件以便能够重现问题。