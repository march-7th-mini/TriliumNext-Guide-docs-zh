...
首次创建 _看板_ 类型的集合时，会创建几个子笔记，每个子笔记都设置了 `#status` 标签。然后看板会根据状态属性的值对每个笔记进行分组。

笔记会递归显示，因此即使是子笔记的子笔记也会被显示。但是，与<a class="reference-link" href="Table.md">表格</a>不同，笔记不会按层级结构显示。

## 交互

### 使用列

*   通过按最后一列附近的 _添加列_ 来创建新列。
    *   按下后，将显示一个文本框以设置列的名称。按 <kbd>Enter</kbd> 确认，或按 <kbd>Escape</kbd> 取消。
*   要重新排序列，只需按住鼠标在标题上并将其拖到所需位置。
*   要删除列，请右键单击其标题并选择 _删除列_。
*   要重命名列，请单击笔记标题。
    *   按 Enter 确认。
    *   重命名列时，其所有笔记的相应状态属性将批量更改。
*   如果列很多，可以使用鼠标滚轮滚动。

### 使用笔记

*   通过按 _新建项目_ 在任何列中创建新笔记。
...
*   每列中笔记的顺序对应于它们在树中的位置。
    *   可以通过在同一列中将笔记拖到所需位置来重新排序。
    *   也可以将笔记跨列拖到所需位置。
*   如需更多选项，请右键单击笔记以显示包含以下选项的上下文菜单：
...
也可以使用<a class="reference-link" href="../Basic%20Concepts%20and%20Features/UI%20Elements/Note%20Tree.md">笔记树</a>在看板上添加项目。
...
*   使用 <kbd>Tab</kbd> 和 <kbd>Shift</kbd>+<kbd>Tab</kbd> 在列标题、笔记和每列的“新建项目”按钮之间按顺序导航。
...
自 v0.100.0 版本起，可以在看板上显示笔记属性，以通过自定义信息增强看板，例如为任务添加 _截止日期_。

此功能仅通过属性定义（<a class="reference-link" href="../Advanced%20Usage/Attributes/Promoted%20Attributes.md">提升属性</a>）实现。添加这些内容的最简单方法是：

1.  转到看板笔记。
2.  在功能区中选择 _拥有的属性_ → 加号按钮 → _添加新的标签/关系定义_。
3.  根据需要配置属性。
4.  勾选 _可继承_ 以使其自动适用于子笔记。

创建属性后，点击一个笔记并填写提升属性，这些属性应反映在看板内。
...
默认情况下，用于对笔记进行分组的标签是 `#status`。如果需要，可以通过定义一个名为 `#board:groupBy` 的标签（其值为要使用的属性，带或不带 `#` 属性前缀）来使用不同的标签。
