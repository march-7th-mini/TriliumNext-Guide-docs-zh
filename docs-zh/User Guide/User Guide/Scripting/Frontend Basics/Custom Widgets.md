...
*   Preact 小组件默认根据内容调整大小，而旧版小组件需要在构造函数中应用 `this.contentSized()`。有关更多信息，请参阅 <a class="reference-link" href="Custom%20Widgets/Troubleshooting.md">故障排除</a> 中的相应章节。
...
2.  将语言设置为：
    ...
    2.  JSX，用于 Preact 小组件。你可能需要先前往 选项 → 代码 以启用该语言。
3.  应用 `#widget` [标签](../../Advanced%20Usage/Attributes/Labels.md)。

## 从简单示例开始

让我们从创建一个在内容区域附近显示消息的小组件开始。按照上一节的说明创建一个代码笔记，并使用以下内容。
...
<table class="ck-table-resized">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>在内容区域中。如果打开了分屏，该小组件将横跨所有分屏。</td>
            <td>参见上面的示例。</td>
            <td>无。</td>
        </tr>
        <tr>
            <th><code spellcheck="false">note-detail-pane</code></th>
            <td><p>在内容区域中，位于笔记详情区域内。如果打开了分屏，该小组件将包含在分屏内。</p><p>如果小组件是特定于笔记的，这是理想的选择。</p></td>
            <td><a class="reference-link" href="Custom%20Widgets/Note%20context%20aware%20widget.md">笔记上下文感知小组件</a></td>
            <td><ul><li>该小组件必须导出 <code spellcheck="false">class</code> 而不是类的实例（例如 <code spellcheck="false">no new</code>），因为它需要为每个笔记进行复制，以便分屏正常工作。</li><li>由于导出的是 <code spellcheck="false">class</code> 而不是实例，<code spellcheck="false">parentWidget</code> getter 必须是 <code spellcheck="false">static</code>，否则该小组件将被忽略。</li></ul></td>
        </tr>
        <tr>
            <th><code spellcheck="false">right-pane</code></th>
            <td>在 <a class="reference-link" href="../../Basic%20Concepts%20and%20Features/UI%20Elements/Right%20Sidebar.md">右侧边栏</a> 中，作为一个专用部分。</td>
...
要将小组件放置在其他位置，只需更改旧版小组件中传递给 `get parentWidget()` 的值，或 Preact 中的 `parent` 字段。请注意，某些位置（如 `note-detail-pane` 和 `right-pane`）有需要处理的特殊要求（参见上表）。
...
启动栏小组件类似于_自定义小组件_，但专门用于 <a class="reference-link" href="../../Basic%20Concepts%20and%20Features/UI%20Elements/Launch%20Bar.md">启动栏</a>。有关更多信息，请参阅 <a class="reference-link" href="Launch%20Bar%20Widgets.md">启动栏小组件</a>。
